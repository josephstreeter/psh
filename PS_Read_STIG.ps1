...
$conn.Open('Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\users\joseph.streeter\desktop\STIGs\STIG.accdb;Persist Security Info=True;')

$rs.Open("SELECT * FROM WS2008R2MS",$conn,$adOpenStatic,$adLockOptimistic)

# Get STIG info from XML
$xml = [xml] (Get-Content c:\users\joseph.streeter\desktop\STIGS\U_Windows_2008_R2_MS_V1R3_STIG_Benchmark-xccdf.xml)
