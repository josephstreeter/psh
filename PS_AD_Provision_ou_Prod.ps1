...
$OrgOU = "ou=" + $OrgCode + ",OU=orgUnits,DC=ad,DC=wisc,DC=edu"
...
New-ADGroup -Name $OrgOUOwners -SamAccountName $OrgOUOwners -GroupCategory Security -GroupScope Global -Path 'OU=orgUnits,OU=Groups,OU=manageDomain,DC=ad,DC=wisc,DC=edu' -Description $OrgName

New-ADGroup -Name $OrgOUAdmins -SamAccountName $OrgOUAdmins -GroupCategory Security -GroupScope Global -Path 'OU=orgUnits,OU=Groups,OU=manageDomain,DC=ad,DC=wisc,DC=edu' -Description $OrgName
...
foreach ($admin in $OrgAdmins)
    ...
    New-ADUser $AdminName -path 'OU=OU-Admins,OU=adminAccounts,OU=manageDomain,DC=ad,DC=wisc,DC=edu' -AccountPassword  (ConvertTo-SecureString -AsPlainText $AdminPassword -Force) -Enabled $true -ChangePasswordAtLogon $true -givenname $NetID.givenname -surname $NetID.sn -displayname $NetID.displayname -UserPrincipalName $AdminUPN -description $orgName -OtherAttributes @{mail=$NetID.mail;telephoneNumber=$NetID.telephoneNumber}
...
New-ADOrganizationalUnit -name $OrgCode -path "OU=orgUnits,DC=ad,DC=wisc,DC=edu" -Description $OrgName -ProtectedFromAccidentalDeletion $true
