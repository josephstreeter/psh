...
Try {
	...
	$NetIDs = Get-ADObject -Filter * -pr altsecurityidentities, memberof -ResultPageSize 1000 -searchbase "ou=NetID,ou=Wisc,DC=$AD_Env,dc=wisc,dc=edu" -ErrorAction Stop
	
	Write-Host -ForegroundColor Yellow "Retrieving all GuestNetIDs`n"
	$GuestNetIDs = Get-ADObject -Filter * -pr altsecurityidentities, memberof -ResultPageSize 1000 -searchbase "ou=GuestNetID,ou=Wisc,DC=$AD_Env,dc=wisc,dc=edu" -ErrorAction Stop
	}
Catch
	{
